...
Rpub: <https://rpubs.com/seungm1nsong/978725>
...
create new column with auto increment named, **customerid** .
```{r}
...
Select only columns that are relevant for our purpose. I am not interested in any personal information for the analysis. Therefore I remove **customer_name** and **customer_email** from my dataframe.
...
It means for a change in one unit in **annual_Salary** will bring 853.83 units to change in **car_purchase_amount**.
```{r}
...
It means for a change in one unit in **annual_Salary** + **net_Worth** will bring 853.83 units to change in **car_purchase_amount**.
```{r}
